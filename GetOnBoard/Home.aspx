<%@ Page Title="" Language="C#" MasterPageFile="~/Master/Site.Master" AutoEventWireup="true" CodeBehind="Home.aspx.cs" Inherits="GetOnBoard.Home" %>
...
<asp:Content ID="Content2" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <div class="playArea right">
        <ul class="toptabs">
            <li class="active" id="tabBoard"><a href="#" id="tabLnkBoard">Board</a></li>
            <li id="tabChats"><a href="#" id="tabLnkChats">Chats</a></li>
            <div class="clear">
            </div>
        </ul>
        <ul id="boardContainer" class="detailstabs">
        </ul>
    </div>
    <!---------------------------Templates---------------------------->
    <div style="display: none">
        ...
        <textarea id="chatTemplate">
            <div class="actionPanel chatBox">
                <table width="100%">
                    <tr>
                        <td width="85%"><textarea id="txtMsg" name="txtMsg" style="width:100%" /></td>
                        <td width="15%" align="right"><input type="button" id="sendMsg" name="sendMsg" style="height:40px; width:70px" value="Send" onclick="gameClient.SendMessage()" /></td>
                    </tr>
                </table>
                <div id="msgBox">
                </div>
        ...
        <textarea  id="msgTemplate">
            {#foreach $T.Chats as chat}
                <div class="msg{#if $T.chat.IsSeen===false} unSeenMsg{#/if}">
                    <table width="100%">
                        <tr>
                            <td width="50%"><b>{#if $T.chat.From===$T.Me}Me{#else}{$T.chat.From}{#/if}</b></td>
                            <td width="50%" align="right"><span class="timestamp">at {$T.chat.TimeStamp}</span></td>
                        </tr>
                        <tr>
                            <td width="50%" colspan="2">{$T.chat.Message}</td>
                        </tr>
                    </table>
                </div>
            {#/for}
            ...
            <table width="100%">
                {#foreach $T.UserGames as game}
                    <tr id="game_{$T.game.Id}" class="actionButton {#if $T.game.Players[0].IsActive && $T.game.Opponant!=''}activeButton{#/if}">
                        <td width="2%" id="ply_{$T.game.Id}_{$T.game.Opponant}"></td>
                        ...
                        <td width="27%">&nbsp{#if $T.game.LastActivityTime}{$T.game.LastActivityTime}{#/if}</td>
