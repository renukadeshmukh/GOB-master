var GameManager = function () {
    ...
    this.RenderGameDetails = function() {
        ...
        if (index != -1) {
            ...
            showMessage("Please select the game.");
        }
    };

    this.RenderChats = function(dayCount) {
        if (!dayCount) {
            dayCount = 0; 
        }
        var gameStatus = gameManager.GameStatus[gameManager.SelGameId];
        if (gameStatus) {
            var fromDays = gameManager.ChatHistory[gameStatus.Player2.Id] ? gameManager.ChatHistory[gameStatus.Player2.Id] : { Days: 0 };
            if (!gameManager.Chats[gameStatus.Player2.UserName] || fromDays.Days < dayCount) {
                gameManager.ChatHistory[gameStatus.Player2.Id] = fromDays;
                gameManager.ChatHistory[gameStatus.Player2.Id].Days = dayCount;
                EnableLoading = true;
                var request = { SessionId: Session.SessionId, Player1: gameStatus.Player1.UserName, Player2: gameStatus.Player2.UserName, FromDays: fromDays.Days };
                $.ajax({
                    type: "POST",
                    contentType: "application/json",
                    data: JSON.stringify(request),
                    url: 'get/AccountService.svc/GetMyChats',
                    dataType: "json",
                    cache: false,
                    success: function(data) {
                        if (data != null && data.IsSucess) {
                            var opponent = gameStatus.Player2.UserName;
                            gameManager.Chats[opponent] = gameManager.Chats[opponent] ? gameManager.Chats[opponent] : [];
                            for (var i = 0; i < data.Chats.length; i++) {
                                var index = findIndex(gameManager.Chats[opponent], "Id", data.Chats[i].Id);
                                if (index === -1) {
                                    gameManager.Chats[opponent].push(data.Chats[i]);
                                } else {
                                    gameManager.Chats[opponent][index] = data.Chats[i];
                                }
                            }
                            actManager.RenderMessages(opponent);
                        } else {
                            showMessage(data && data.ErrorMessage ? "GetMyChats: " + data.ErrorMessage : "Faild to get your messages! Unknown error.");
                        }
                    },
                    error: function() {
                        showMessage('Unknown error please report.');
                ...
                actManager.RenderMessages(gameStatus.Player2.UserName);
            }
        }
    };
