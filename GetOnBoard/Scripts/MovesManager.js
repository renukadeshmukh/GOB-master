var MovesManager = function () {
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data != null && data.IsSucess && data.Move && data.Game) {
                        gameClient.NotifyTurn(gameManager.GameStatus[data.Game.Id].Player2.UserName, data.Game.Id);
    ...
    this.StartMovesPolling = function() {
        if (!gameManager.MM.GameMovesTimer) {
            gameManager.MM.GameMovesTimer = setInterval("gameManager.MM.PollGameMoves();", 60000);
        }
    };
};